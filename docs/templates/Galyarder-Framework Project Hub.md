# Galyarder Framework Project Hub

## Core Vision
- [[Galyarder Framework Vision - Digital Company for Solo Founders]]
- [[Galyarder Framework Department Map]]
- [[Galyarder Framework Agent-to-Skill Mapping]]
- [[Galyarder Framework Current Gaps and Hiring Plan]]

## Reporting and Operating Model
- [[Galyarder Framework Founder Report Templates]]
- [[Galyarder Framework Linear Operating Model]]
- [[Galyarder Framework Initial Linear Backlog]]
- [[Galyarder Framework Linear Ticket Specs]]
- [[Galyarder Framework Linear Import Pack]]
- [[Galyarder Framework Decision Log Template]]
- [[Galyarder Framework Milestone Summary Template]]

## Department Reports
- [[Galyarder Framework Department Reports/Department Report Index]]

## Standard Audits
- [[Galyarder Framework Engineering Audit]]
- [[Galyarder Framework Growth Audit]]
- [[Galyarder Framework Founder Office Audit]]
- [[Galyarder Framework Product Audit]]
- [[Galyarder Framework Operations Audit]]
- [[Galyarder Framework Security Audit]]
- [[Galyarder Framework Legal-Finance Audit]]
- [[Galyarder Framework Knowledge Audit]]
- [[Galyarder Framework Cross-Department Hire Queue]]
- [[Galyarder Framework Import Adapt Reject Rubric]]

## Suggested Reading Order
1. Read the vision and department map.
2. Read the current gaps and hiring plan.
3. Review the Standard audits by department.
4. Use the hire queue and rubric to choose imports.
5. Convert approved hires into Linear workstreams and Obsidian decision records.

## Current Priority
- define the first wave of imported or adapted hires
- turn Standard audits into actual implementation backlog
- make reporting and memory systems mandatory for every department
