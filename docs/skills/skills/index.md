---
title: "Technical Skills Skills  Agent Skills & Codex Plugins"
description: "141 technical skills skills  high-integrity technical skills for TDD, SEO, CRO, and digital company orchestration. Works with Claude Code, Codex CLI, Gemini CLI, and OpenClaw."
---

<div class="domain-header" markdown>

#  Technical Skills

<p class="domain-count">141 skills in this domain</p>

</div>

<div class="install-banner" markdown>
<span class="install-label">Install all:</span> <code>claude /plugin install master-skills</code>
</div>

<div class="grid cards" markdown>

-   **[Ab Test Setup](ab-test-setup.md)**

    ---

    You are the Ab Test Setup Specialist at Galyarder Labs.

-   **[Accelerator Application](accelerator-application.md)**

    ---

    You are the Accelerator Application Specialist at Galyarder Labs.

-   **[Accounting](accounting.md)**

    ---

    You are the Accounting Specialist at Galyarder Labs.

-   **[Adversarial Reviewer](adversarial-reviewer.md)**

    ---

    You are the Adversarial Reviewer Specialist at Galyarder Labs.

-   **[Analytics Architect](analytics-architect.md)**

    ---

    You are the Analytics Architect Specialist at Galyarder Labs.

-   **[Analytics Tracking](analytics-tracking.md)**

    ---

    You are the Analytics Tracking Specialist at Galyarder Labs.

-   **[Architect](architect.md)**

    ---

    You are the Architect Specialist at Galyarder Labs.

-   **[Audit Website](audit-website.md)**

    ---

    You are the Audit Website Specialist at Galyarder Labs.

-   **[Board Deck Builder](board-deck-builder.md)**

    ---

    You are the Board Deck Builder Specialist at Galyarder Labs.

-   **[Board Update](board-update.md)**

    ---

    You are the Board Update Specialist at Galyarder Labs.

-   **[Brainstorming](brainstorming.md)**

    ---

    You are the Brainstorming Specialist at Galyarder Labs.

-   **[Build Error Resolver](build-error-resolver.md)**

    ---

    You are the Build Error Resolver Specialist at Galyarder Labs.

-   **[Step 1  Attribution: understand which channels drive conversions](campaign-analytics.md)**

    ---

    You are the Campaign Analytics Specialist at Galyarder Labs.

-   **[Analyze an IAM policy for privilege escalation paths](cloud-security.md)**

    ---

    You are the Cloud Security Specialist at Galyarder Labs.

-   **[Code Review Expert](code-review-expert.md)**

    ---

    You are the Code Review Expert Specialist at Galyarder Labs.

-   **[Code Reviewer](code-reviewer.md)**

    ---

    code-reviewer

-   **[[competitor].yaml](competitor-alternatives.md)**

    ---

    You are the Competitor Alternatives Specialist at Galyarder Labs.

-   **[Content Creator](content-creator.md)**

    ---

    You are the Content Creator Specialist at Galyarder Labs.

-   **[Content Strategy](content-strategy.md)**

    ---

    You are the Content Strategy Specialist at Galyarder Labs.

-   **[Contract And Proposal Writer](contract-and-proposal-writer.md)**

    ---

    You are the Contract And Proposal Writer Specialist at Galyarder Labs.

-   **[Contract Review](contract-review.md)**

    ---

    You are the Contract Review Specialist at Galyarder Labs.

-   **[Conversion Engineer](conversion-engineer.md)**

    ---

    You are the Conversion Engineer Specialist at Galyarder Labs.

-   **[Copy Editing](copy-editing.md)**

    ---

    You are the Copy Editing Specialist at Galyarder Labs.

-   **[Copywriting](copywriting.md)**

    ---

    You are the Copywriting Specialist at Galyarder Labs.

-   **[Cyber Intel](cyber-intel.md)**

    ---

    You are the Cyber Intel Specialist at Galyarder Labs.

-   **[Data Room](data-room.md)**

    ---

    You are the Data Room Specialist at Galyarder Labs.

-   **[Decision Logger](decision-logger.md)**

    ---

    You are the Decision Logger Specialist at Galyarder Labs.

-   **[Defuddle](defuddle.md)**

    ---

    You are the Defuddle Specialist at Galyarder Labs.

-   **[Devops Engineer](devops-engineer.md)**

    ---

    You are the Devops Engineer Specialist at Galyarder Labs.

-   **[Dispatching Parallel Agents](dispatching-parallel-agents.md)**

    ---

    You are the Skill at Galyarder Labs.

-   **[Doc Updater](doc-updater.md)**

    ---

    You are the Doc Updater Specialist at Galyarder Labs.

-   **[E2E Runner](e2e-runner.md)**

    ---

    You are the E2E Runner Specialist at Galyarder Labs.

-   **[THE ELITE DEVELOPER: PRINCIPAL ENGINEERING PROTOCOL](elite-developer.md)**

    ---

    You are the Elite Developer Specialist at Galyarder Labs.

-   **[Email Marketing Bible](email-marketing-bible.md)**

    ---

    You are the Email Marketing Bible Specialist at Galyarder Labs.

-   **[Eradicating Malware From Infected Systems](eradicating-malware-from-infected-systems.md)**

    ---

    You are the Eradicating Malware From Infected Systems Specialist at Galyarder Labs.

-   **[Executing Active Directory Attack Simulation](executing-active-directory-attack-simulation.md)**

    ---

    You are the Executing Active Directory Attack Simulation Specialist at Galyarder Labs.

-   **[Executing Phishing Simulation Campaign](executing-phishing-simulation-campaign.md)**

    ---

    You are the Executing Phishing Simulation Campaign Specialist at Galyarder Labs.

-   **[Executing Plans](executing-plans.md)**

    ---

    You are the Executing Plans Specialist at Galyarder Labs.

-   **[Executing Red Team Engagement Planning](executing-red-team-engagement-planning.md)**

    ---

    You are the Executing Red Team Engagement Planning Specialist at Galyarder Labs.

-   **[Executing Red Team Exercise](executing-red-team-exercise.md)**

    ---

    You are the Executing Red Team Exercise Specialist at Galyarder Labs.

-   **[Experimentation Engineer](experimentation-engineer.md)**

    ---

    You are the Experimentation Engineer Specialist at Galyarder Labs.

-   **[Finance Based Pricing Advisor](finance-based-pricing-advisor.md)**

    ---

    finance-based-pricing-advisor

-   **[Financial Analyst](financial-analyst.md)**

    ---

    You are the Financial Analyst Specialist at Galyarder Labs.

-   **[Finishing A Development Branch](finishing-a-development-branch.md)**

    ---

    You are the Finishing A Development Branch Specialist at Galyarder Labs.

-   **[Finops Manager](finops-manager.md)**

    ---

    You are the Finops Manager Specialist at Galyarder Labs.

-   **[Founder Context](founder-context.md)**

    ---

    You are the Founder Context Specialist at Galyarder Labs.

-   **[Founder Thought Leadership](founder-thought-leadership.md)**

    ---

    You are the Founder Thought Leadership Specialist at Galyarder Labs.

-   **[Fundraising Email](fundraising-email.md)**

    ---

    You are the Fundraising Email Specialist at Galyarder Labs.

-   **[Galyarder Specialist](galyarder-specialist.md)**

    ---

    You are the Master Orchestrator of the Galyarder Framework. You possess deep knowledge of the entire Digital Company ...

-   **[Gdpr Ccpa Privacy Auditor](gdpr-ccpa-privacy-auditor.md)**

    ---

    You are the Gdpr Ccpa Privacy Auditor Specialist at Galyarder Labs.

-   **[Gdpr Compliance](gdpr-compliance.md)**

    ---

    You are the Gdpr Compliance Specialist at Galyarder Labs.

-   **[Generating Threat Intelligence Reports](generating-threat-intelligence-reports.md)**

    ---

    You are the Generating Threat Intelligence Reports Specialist at Galyarder Labs.

-   **[Growth Engineer](growth-engineer.md)**

    ---

    You are the Growth Engineer Specialist at Galyarder Labs.

-   **[THE GROWTH STRATEGIST: CMO PROTOCOL](growth-strategist.md)**

    ---

    You are the Growth Strategist Specialist at Galyarder Labs.

-   **[Intercepting Mobile Traffic With Burpsuite](intercepting-mobile-traffic-with-burpsuite.md)**

    ---

    You are the Intercepting Mobile Traffic With Burpsuite Specialist at Galyarder Labs.

-   **[Interface Designer](interface-designer.md)**

    ---

    You are the Interface Designer Specialist at Galyarder Labs.

-   **[Investigating Phishing Email Incident](investigating-phishing-email-incident.md)**

    ---

    You are the Investigating Phishing Email Incident Specialist at Galyarder Labs.

-   **[Investor Research](investor-research.md)**

    ---

    You are the Investor Research Specialist at Galyarder Labs.

-   **[ISO 42001 AI Governance Audit Report](iso-42001-ai-governance.md)**

    ---

    You are the Iso 42001 Ai Governance Specialist at Galyarder Labs.

-   **[Json Canvas](json-canvas.md)**

    ---

    You are the Json Canvas Specialist at Galyarder Labs.

-   **[Launch Strategy](launch-strategy.md)**

    ---

    You are the Launch Strategy Specialist at Galyarder Labs.

-   **[Lead Scoring](lead-scoring.md)**

    ---

    You are the Lead Scoring Specialist at Galyarder Labs.

-   **[Legal Advisor](legal-advisor.md)**

    ---

    legal-advisor

-   **[Legal Counsel](legal-counsel.md)**

    ---

    You are the Legal Counsel Specialist at Galyarder Labs.

-   **[Package/project metadata](legal-tos-privacy.md)**

    ---

    You are the Legal Tos Privacy Specialist at Galyarder Labs.

-   **[Mapping Mitre Attack Techniques](mapping-mitre-attack-techniques.md)**

    ---

    You are the Mapping Mitre Attack Techniques Specialist at Galyarder Labs.

-   **[Market Research](market-research.md)**

    ---

    You are the Market Research Specialist at Galyarder Labs.

-   **[Marketing Demand Acquisition](marketing-demand-acquisition.md)**

    ---

    You are the Marketing Demand Acquisition Specialist at Galyarder Labs.

-   **[Marketing Ideas](marketing-ideas.md)**

    ---

    You are the Marketing Ideas Specialist at Galyarder Labs.

-   **[Marketing Psychology](marketing-psychology.md)**

    ---

    You are the Marketing Psychology Specialist at Galyarder Labs.

-   **[Monitoring Darkweb Sources](monitoring-darkweb-sources.md)**

    ---

    You are the Monitoring Darkweb Sources Specialist at Galyarder Labs.

-   **[Obsidian Bases](obsidian-bases.md)**

    ---

    You are the Obsidian Bases Specialist at Galyarder Labs.

-   **[Obsidian Cli](obsidian-cli.md)**

    ---

    You are the Obsidian Cli Specialist at Galyarder Labs.

-   **[Obsidian Markdown](obsidian-markdown.md)**

    ---

    You are the Obsidian Markdown Specialist at Galyarder Labs.

-   **[Onboarding Cro](onboarding-cro.md)**

    ---

    You are the Onboarding Cro Specialist at Galyarder Labs.

-   **[Open Source License Skill](open-source-license.md)**

    ---

    You are the Open Source License Specialist at Galyarder Labs.

-   **[Page Cro](page-cro.md)**

    ---

    You are the Page Cro Specialist at Galyarder Labs.

-   **[Paywall Upgrade Cro](paywall-upgrade-cro.md)**

    ---

    You are the Paywall Upgrade Cro Specialist at Galyarder Labs.

-   **[Perseus](perseus.md)**

    ---

    You are the Perseus Specialist at Galyarder Labs.

-   **[Pitch Deck](pitch-deck.md)**

    ---

    You are the Pitch Deck Specialist at Galyarder Labs.

-   **[Planner](planner.md)**

    ---

    planner

-   **[1. Generate tests from a user story](playwright-pro.md)** + 9 sub-skills

    ---

    You are the Playwright Pro Specialist at Galyarder Labs.

-   **[View diff in terminal](pr-review-expert.md)**

    ---

    You are the Pr Review Expert Specialist at Galyarder Labs.

-   **[Prd To Issues](prd-to-issues.md)**

    ---

    You are the Prd To Issues Specialist at Galyarder Labs.

-   **[Prd To Plan](prd-to-plan.md)**

    ---

    You are the Prd To Plan Specialist at Galyarder Labs.

-   **[Pricing Strategy](pricing-strategy.md)**

    ---

    You are the Pricing Strategy Specialist at Galyarder Labs.

-   **[Product Manager](product-manager.md)**

    ---

    You are the Product Manager Specialist at Galyarder Labs.

-   **[Product Marketing Context](product-marketing-context.md)**

    ---

    You are the Product Marketing Context Specialist at Galyarder Labs.

-   **[Profiling Threat Actor Groups](profiling-threat-actor-groups.md)**

    ---

    You are the Profiling Threat Actor Groups Specialist at Galyarder Labs.

-   **[Programmatic SEO](programmatic-seo.md)**

    ---

    You are the Programmatic Seo Specialist at Galyarder Labs.

-   **[Qa Automation Engineer](qa-automation-engineer.md)**

    ---

    You are the Qa Automation Engineer Specialist at Galyarder Labs.

-   **[Receiving Code Review](receiving-code-review.md)**

    ---

    You are the Receiving Code Review Specialist at Galyarder Labs.

-   **[Recovering Deleted Files With Photorec](recovering-deleted-files-with-photorec.md)**

    ---

    You are the Recovering Deleted Files With Photorec Specialist at Galyarder Labs.

-   **[Recovering From Ransomware Attack](recovering-from-ransomware-attack.md)**

    ---

    You are the Recovering From Ransomware Attack Specialist at Galyarder Labs.

-   **[Refactor Cleaner](refactor-cleaner.md)**

    ---

    You are the Refactor Cleaner Specialist at Galyarder Labs.

-   **[Referral Program](referral-program.md)**

    ---

    You are the Referral Program Specialist at Galyarder Labs.

-   **[Release Manager](release-manager.md)**

    ---

    You are the Release Manager Specialist at Galyarder Labs.

-   **[Remotion Best Practices](remotion-best-practices.md)**

    ---

    remotion-best-practices

-   **[Remotion Engineer](remotion-engineer.md)**

    ---

    You are the Remotion Engineer Specialist at Galyarder Labs.

-   **[Requesting Code Review](requesting-code-review.md)**

    ---

    You are the Requesting Code Review Specialist at Galyarder Labs.

-   **[Retention Specialist](retention-specialist.md)**

    ---

    You are the Retention Specialist Specialist at Galyarder Labs.

-   **[Revenue Architect](revenue-architect.md)**

    ---

    You are the Revenue Architect Specialist at Galyarder Labs.

-   **[Reverse Engineering Malware With Ghidra](reverse-engineering-malware-with-ghidra.md)**

    ---

    You are the Reverse Engineering Malware With Ghidra Specialist at Galyarder Labs.

-   **[Print runbook to stdout](runbook-generator.md)**

    ---

    You are the Runbook Generator Specialist at Galyarder Labs.

-   **[Saas Finops Optimization](saas-finops-optimization.md)**

    ---

    You are the Saas Finops Optimization Specialist at Galyarder Labs.

-   **[Schema Markup & Structured Data](schema-markup.md)**

    ---

    You are the Schema Markup Specialist at Galyarder Labs.

-   **[Security Guardian](security-guardian.md)**

    ---

    You are the Security Guardian Specialist at Galyarder Labs.

-   **[Security Reviewer](security-reviewer.md)**

    ---

    You are the Security Reviewer Specialist at Galyarder Labs.

-   **[Seo Audit](seo-audit.md)**

    ---

    You are the Seo Audit Specialist at Galyarder Labs.

-   **[Signup Flow Cro](signup-flow-cro.md)**

    ---

    You are the Signup Flow Cro Specialist at Galyarder Labs.

-   **[Social Content](social-content.md)**

    ---

    You are the Social Content Specialist at Galyarder Labs.

-   **[Social Strategist](social-strategist.md)**

    ---

    You are the Social Strategist Specialist at Galyarder Labs.

-   **[Subagent Driven Development](subagent-driven-development.md)**

    ---

    You are the Subagent Driven Development Specialist at Galyarder Labs.

-   **[THE SUPER ARCHITECT: SYSTEMS DESIGN PROTOCOL](super-architect.md)**

    ---

    You are the Super Architect Specialist at Galyarder Labs.

-   **[Support Lead](support-lead.md)**

    ---

    You are the Support Lead Specialist at Galyarder Labs.

-   **[Systematic Debugging](systematic-debugging.md)**

    ---

    You are the Systematic Debugging Specialist at Galyarder Labs.

-   **[Tdd Guide](tdd-guide.md)**

    ---

    You are the Tdd Guide Specialist at Galyarder Labs.

-   **[Test Driven Development](test-driven-development.md)**

    ---

    You are the Test Driven Development Specialist at Galyarder Labs.

-   **[Testing For Xss Vulnerabilities With Burpsuite](testing-for-xss-vulnerabilities-with-burpsuite.md)**

    ---

    You are the Testing For Xss Vulnerabilities With Burpsuite Specialist at Galyarder Labs.

-   **[Tracking Threat Actor Infrastructure](tracking-threat-actor-infrastructure.md)**

    ---

    You are the Tracking Threat Actor Infrastructure Specialist at Galyarder Labs.

-   **[Ubiquitous Language](ubiquitous-language.md)**

    ---

    You are the Ubiquitous Language Specialist at Galyarder Labs.

-   **[THE UI/UX DESIGNER: HEAD OF DESIGN PROTOCOL](ui-ux-designer.md)**

    ---

    You are the Ui Ux Designer Specialist at Galyarder Labs.

-   **[Using Galyarder Framework](using-galyarder-framework.md)**

    ---

    You are the Using Galyarder Framework Specialist at Galyarder Labs.

-   **[Using Git Worktrees](using-git-worktrees.md)**

    ---

    You are the Using Git Worktrees Specialist at Galyarder Labs.

-   **[Validating Backup Integrity For Recovery](validating-backup-integrity-for-recovery.md)**

    ---

    You are the Validating Backup Integrity For Recovery Specialist at Galyarder Labs.

-   **[Vercel React Best Practices](vercel-react-best-practices.md)**

    ---

    You are the Vercel React Best Practices Specialist at Galyarder Labs.

-   **[Vercel React Expert](vercel-react-expert.md)**

    ---

    You are the Vercel React Expert Specialist at Galyarder Labs.

-   **[Verification Before Completion](verification-before-completion.md)**

    ---

    You are the Verification Before Completion Specialist at Galyarder Labs.

-   **[Web Design Guidelines](web-design-guidelines.md)**

    ---

    You are the Web Design Guidelines Specialist at Galyarder Labs.

-   **[Write A Prd](write-a-prd.md)**

    ---

    write-a-prd

-   **[Writing Plans](writing-plans.md)**

    ---

    You are the Writing Plans Specialist at Galyarder Labs.

-   **[Writing Skills](writing-skills.md)**

    ---

    You are the Writing Skills Specialist at Galyarder Labs.

</div>
