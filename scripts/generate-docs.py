...
GITHUB_BASE = "https://github.com/galyarderlabs/galyarder-framework/tree/main"
...
<span class="meta-badge"> `{skill["name"]}`</span>
<span class="meta-badge"> <a href="https://github.com/galyarderlabs/galyarder-framework/tree/main/{skill["rel_path"]}/SKILL.md">Source</a></span>
</div>
...
'''
    ...
    def process_agent(base_dir, agent_file, domain_label, domain_icon):
        ...
        title = re.sub(r"[*_`]", "", title)
        if re.match(r"^cs-[a-z-]+$", title): title = prettify(title.removeprefix("cs-"))
        with open(agent_path, "r", encoding="utf-8") as f: content = f.read()
        content_clean = strip_content(content)
        content_clean = rewrite_relative_links(content_clean, rel)
        agent_seo_title = f"{title} | AI Agent & Galyarder Framework"
        agent_fm_desc = extract_description_from_frontmatter(agent_path)
        if agent_fm_desc:
            agent_clean = agent_fm_desc.strip("'\"").replace('"', "'")
            if len(agent_clean) > 150: agent_clean = agent_clean[:150].rsplit(" ", 1)[0].rstrip(".,;:-")
            agent_desc = f"{agent_clean}. Agent-native orchestrator for Galyarder Framework."
        else: agent_desc = f"{title} agent for Galyarder Labs orchestration."
        page = f'''---\ntitle: "{agent_seo_title}"\ndescription: "{agent_desc}"\n---\n\n<div class="domain-header" markdown>\n\n# {domain_icon} {title}\n\n<p class="domain-label">{domain_label} Agent</p>\n\n</div>\n\n{content_clean}'''
    ...
    if os.path.isdir(agents_dir):
        for item in sorted(os.listdir(agents_dir)):
            ...
            if os.path.isdir(item_path):
                domain_info = AGENT_DOMAINS.get(item, (prettify(item), ""))
                for agent_file in sorted(os.listdir(item_path)):
                    if agent_file.endswith(".md"): process_agent(item_path, agent_file, domain_info[0], domain_info[1])
            elif item.endswith(".md") and item != "README.md":
                process_agent(agents_dir, item, "Operations", "")
    if agent_entries:
        agent_cards = ""
...
<span class="meta-badge"> <a href="https://github.com/galyarderlabs/galyarder-framework/tree/main/{rel}">Source</a></span>
</div>
